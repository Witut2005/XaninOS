...
# [name, type, first_sector, sectros_occupied]
...
for entry in xanin_entries_to_preinstall:
    ...
    if entry[1] != XIN_DIRECTORY:
        file.seek(xin_filesystem_pointers_begin + entry[FIRST_SECTOR])
        for sectors_occupied in range(0, entry[SECTORS_OCCUPIED] - 1):
    ...
    file.write(bytes(entry[TYPE], 'ascii'))
    file.write(bytes(13))
