...
void scanf(char* str, ... )
{
    ...
    uint8_t starting_x = Screen.x + 1;
    uint8_t starting_y = Screen.y + 1;

    uint8_t current_x = starting_x;
    uint8_t current_y = starting_y;
    ...
    while(true)
    {
        ...
        if(KeyInfo.is_bspc)
        {
            ...
            if(&Screen.cursor[Screen.y][Screen.x - 1] == &Screen.cursor[starting_y][starting_x]
                 && character_blocked != '\0')
            {
                goto start;
            }
                ...
                if(str[str_counter] == '%')
                {
                    ...
                    switch(str[str_counter])
                    {
                        case 's':
                        {
                            ...
                            for(int i = 0x0; (char)(Screen.cursor[current_y][current_x]) != '\0' 
                                    && (char)(Screen.cursor[current_y][current_x] != ' '); i++)
                            {
                                buffer[i] = (char)Screen.cursor[current_y][current_x];
                                current_y++;
                                current_x++;
                                counter++;
                            }
