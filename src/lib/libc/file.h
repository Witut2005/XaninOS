...
extern "C" {
#endif
static inline XinEntry* fopen(char const* path, char* mode)
{
    return (XinEntry*)xanin_syscall2(XANIN_FOPEN, (uint32_t)path, (uint32_t)mode);
}
...
static inline int open(char const* path, uint32_t mode) { return (int)xanin_syscall2(XANIN_OPEN, (uint32_t)path, (uint32_t)mode); }
