...
Xtf* xtf_init(uint32_t buffer_size)
{
    ...
    XtFrontend->buffer = (XtCell*)calloc(XtFrontend->size_allocated * sizeof(XtCell));
...
{
    char const* buffer = (char*)XtFrontend->buffer;
    int lines_total = 0;
...
void __xtf_cell_put(Xtf* XtFrontend, char c, uint8_t color)
{
    ...
    {
        XtFrontend->buffer = (XtCell*)realloc(XtFrontend->buffer, (XtFrontend->size + SECTOR_SIZE * 4) * sizeof(XtCell));
