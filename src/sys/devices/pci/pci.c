    ...
    const uint8_t function)
{
    ...
    address = (uint32_t)((bus << 16) | (slot << 11) | (function << 8) | (0x0) | ((uint32_t)0x80000000));
    ...
    const uint8_t function, const uint8_t offset)
{
    ...
    address = (uint32_t)((bus << 16) | (slot << 11) | (function << 8) | (offset));
    ...
    const uint8_t function)
{
    ...
    address = (uint32_t)((bus << 16) | (slot << 11) | (function << 8) | (0x8) | (uint32_t)ENABLE_CONFIGURATION_SPACE_MAPPING);
...
uint32_t pci_get_bar(const uint8_t bus, const uint8_t slot, const uint8_t function, const uint8_t bar_number)
{
    ...
    address = (uint32_t)((bus << 16) | (slot << 11) | (function << 8) | (0x10 + 0x4 * bar_number) | (uint32_t)ENABLE_CONFIGURATION_SPACE_MAPPING);
    ...
    const uint8_t function, const uint8_t offset)
{
    ...
    address = (uint32_t)((bus << 16) | (slot << 11) | (function << 8) | (offset & 0xFC) | (uint32_t)ENABLE_CONFIGURATION_SPACE_MAPPING);
...
uint8_t pci_get_data8(const uint8_t bus, const uint8_t slot, const uint8_t function, const uint8_t offset)
{
    ...
    address = (uint32_t)((bus << 16) | (slot << 11) | (function << 8) | (offset & 0xFC) | ENABLE_CONFIGURATION_SPACE_MAPPING);
...
uint16_t pci_write_data16(const uint8_t bus, const uint8_t slot, const uint8_t function,
    const uint8_t offset, const uint16_t value)
{

    // pci_set_parameters(pci_config_address, configuration_address);

    const uint32_t shift = ((offset & 0x3) * 8);
    uint32_t address;
    uint16_t ret;

#warning "TODO pci driver cpp sus if";
    if ((offset & 0x3) <= 2)
    {
        address = (uint32_t)((bus << 16) | (slot << 11) | (function << 8) | (offset & 0xFC) | ENABLE_CONFIGURATION_SPACE_MAPPING);

        outdIO(PCI_ADDRESS_PORT, address);
        outdIO(PCI_DATA_PORT, (indIO(PCI_DATA_PORT) & ~(0xFFFF << shift)) | value << shift);
    }

    else
    {
        pci_write_data8(bus, slot, function, offset, value & 0xFF);
        pci_write_data8(bus, slot, function, offset + 1, value >> 8);
    }
...
void pci_write_data32(uint32_t configuration_address, uint8_t register_id, uint32_t value)
{
    ...
    address = (uint32_t)((pci_config_address.pci_bus_number << 16) | (pci_config_address.pci_device_number << 11) | (pci_config_address.pci_function_number << 8) | (register_id & 0xFC) | (uint32_t)ENABLE_CONFIGURATION_SPACE_MAPPING);
