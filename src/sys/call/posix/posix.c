...
uint32_t syscall_handle(void)
{
    ...
    case __NR_rename: {
        __xin_entry_move((char*)ebx, (char*)ecx);
        break;
    }

    case __NR_rmdir: {
        __xin_folder_remove((char*)ebx);
        break;
    }
    ...
    case __NR_link: {
        eax = __xin_link_create((char*)ebx, (char*)ecx);
        // xprintf("filename: %s\n", (char*)ebx);
        // xprintf("linkname: %s\n", (char*)ecx);
        break;
    }
    ...
    case __NR_reboot: {
        if (ebx == LINUX_REBOOT_MAGIC1 && (ecx == LINUX_REBOOT_MAGIC2 || ecx == LINUX_REBOOT_MAGIC2A || ecx == LINUX_REBOOT_MAGIC2B || ecx == LINUX_REBOOT_MAGIC2C))
        {
            reboot();
        }
    ...
    case __NR_unlink: {
        eax = __xin_link_remove((char*)ebx);
        break;
    }
